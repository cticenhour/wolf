...
{
  return -_mobility * _u[_qp] * _electron_density[_qp] * _grad_potential[_qp] * _grad_test[_i][_qp];
}

Real
EFieldAdvectionEnergy::computeQpJacobian()
{
  return -_mobility * _phi[_j][_qp] * _electron_density[_qp] * _grad_potential[_qp] *
         _grad_test[_i][_qp];
}
...
EFieldAdvectionEnergy::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _potential_id)
    return -_mobility * _u[_qp] * _electron_density[_qp] * _grad_phi[_j][_qp] * _grad_test[_i][_qp];

  else if (jvar == _electron_id)
    return -_mobility * _u[_qp] * _phi[_j][_qp] * _grad_potential[_qp] * _grad_test[_i][_qp];
