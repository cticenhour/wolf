...
{
  return _grad_test[_i][_qp] * _diffusivity * _electron_density[_qp] * _grad_u[_qp];
}

Real
CoupledEnergyDiffusion::computeQpJacobian()
{
  return _grad_test[_i][_qp] * _diffusivity * _electron_density[_qp] * _grad_phi[_j][_qp];
}
...
CoupledEnergyDiffusion::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _electron_id)
    return _grad_test[_i][_qp] * _diffusivity * _phi[_j][_qp] * _grad_u[_qp];
